...
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
CString CMD5Checksum::GetFileMD5(const CString& strFilePath)
{
...
*****************************************************************************************/
CString CMD5Checksum::GetBlockMD5(const BYTE* pBuf, UINT nLength)
{
	//entry invariants
...
CString CMD5Checksum::GetStringMD5(LPCTSTR lpInput)
{
	CString s;
	const DWORD LEN = _tcslen(lpInput) * sizeof(TCHAR);
	BYTE* p = new BYTE[LEN];
	memcpy(p, lpInput, LEN);
	s = GetBlockMD5(p, LEN);
	delete [] p;
	return s;
}
